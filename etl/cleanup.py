...
def apply_enum_mappings(df, node_name, mappings):
    ...
    if global_missing:
        expanded_global = {}
        for raw, gen3 in global_missing.items():
            expanded_global[raw] = gen3 if isinstance(raw, int) else gen3 
            expanded_global[str(raw)] = gen3
            expanded_global[f"{float(raw):.1f}"] = gen3

        for col in df.columns:
            df[col] = df[col].replace(expanded_global)
...
def strip_whitespace_and_blanks(df):
    """Strip whitespace from all strings and replace blank strings with NaN."""
    df = df.copy()
    
    # 1. Strip leading and trailing spaces from all text columns
    for col in df.select_dtypes(include=['object']).columns:
        df[col] = df[col].str.strip()
        
    # 2. Replace empty strings with NaN so fillna works correctly downstream
...
def process_individuals_table(raw_path, valid_ids, mappings):
    ...
    # Strip trailing .0 from mixed-type demographic columns
    eth_cols = ["ethnicity", "father_ethnicity", "mother_ethnicity", "mat_gfather_ethnicity", "mat_gmother_ethnicity"]
    
    for col in eth_cols:
        if col in df_demo.columns:
            # Convert to string, strip the .0 at the end, and replace literal "nan" back to actual NaN
            df_demo[col] = (
                df_demo[col]
                .astype(str)
                .str.replace(r"\.0$", "", regex=True)
...
def process_template_nodes(raw_path, template_mapping, valid_ids, mappings):
    ...
    master_subjects = pd.read_json("payloads/payload_subject.json")
    ...
    for node_name, template_path in template_mapping.items():
        ...
        if "obs_date" in df_node.columns:
            df_node["obs_date_str"] = df_node["obs_date"].fillna("unknown_date").astype(str)
            df_node["submitter_id"] = node_name + "-" +df_node["individual_id"].astype(str) + "-" + df_node["obs_date_str"]
            df_node = df_node.drop(columns=["obs_date_str"])
        else:
            df_node["submitter_id"] = df_node["individual_id"].apply(
                lambda x: f"{node_name}-{x}-{uuid.uuid4().hex[:8]}"
            )
        ...
        # 1. Global replacements across all columns in this node
        replace_dict = {
            -555: '888-not applicable',
            555: '888-not applicable',
            -555.0: '888-not applicable',
            '-555': '888-not applicable',
            '555': '888-not applicable',
            '-555.0': '888-not applicable',
            -8: '444-refused',
        ...
        for col in df_node.columns:
            # Pass 1: Standard exact replacement (catches clean numbers and strings)
            df_node[col] = df_node[col].replace(replace_dict)
            
            # Pass 2: Regex replacement (catches string versions with hidden spaces)
            if df_node[col].dtype == object:
                 df_node[col] = df_node[col].replace(r'^\s*-?555(\.0)?\s*$', '888-not applicable', regex=True)      

        # 2. Specific anomalous entries
        if 'waist_cm' in df_node.columns:
            df_node['waist_cm'] = df_node['waist_cm'].replace({
                -99.9: '999-missing',
                -99: '999-missing',
                -88.8: '888-not applicable'
            })

        # 2b. Map 2.0 / 2 to "333-don't know" only for specific target variables
        target_333_cols = [
            "hpt_12m", "hpt_rx_trad", "canc_brst_ever", 
            "chol_ever", "diab_mom", "kidney_low_func", "hpt_rx_current"
        ]
        dont_know_replace = {
            2: "333-don't know",
            2.0: "333-don't know",
        ...
        for target_col in target_333_cols:
            if target_col in df_node.columns:
                df_node[target_col] = df_node[target_col].replace(dont_know_replace)
                if df_node[target_col].dtype == object:
                    df_node[target_col] = df_node[target_col].replace(
                        r'^\s*2(\.0)?\s*$', "333-don't know", regex=True
                    )

        df_node = apply_enum_mappings(df_node, node_name, mappings)

        # 3. Fill remaining missing/blank values with the standard 999 code
        structural_cols = ["subjects.submitter_id", "submitter_id", "type"]
        fillable_cols = [c for c in df_node.columns if c not in structural_cols]
        df_node[fillable_cols] = df_node[fillable_cols].fillna("999-missing")

        out_path = f"payloads/payload_{node_name}.json"
        
        # 1. Count occurrences of each submitter_id (0 for first, 1 for second, etc.)
        df_node['seq'] = df_node.groupby('submitter_id').cumcount().astype(str)

        # 2. Append the sequence number to the submitter_id
        df_node['submitter_id'] = df_node['submitter_id'] + "-" + df_node['seq']

        # 3. Clean up the temporary column
        df_node = df_node.drop(columns=['seq'])
